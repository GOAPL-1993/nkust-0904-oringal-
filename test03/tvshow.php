...
playlist
  ...
  if ($result->num_rows > 0) { //檢查記錄的數量，看看是否有資料
    ...
    //以下是表格標題列
    if ($user_type == NULL)
      echo "<tr bgcolor=#cccccc><td>影片標題</td><td>影片ID</td></tr>";
    else
      //如果login的話，才能使用貼文管理
      echo "<tr bgcolor=#cccccc><td>影片標題</td><td>影片ID</td><td>貼文管理</td></tr>";
    while ($row = $result->fetch_assoc()) {
      $id = $row["id"]; //指定各訊息的id
      echo "<tr bgcolor=pink>";
      echo "<td><a href=tvshow.php?pid=$id>" . $row["title"] . "</a></td>";
      echo "<td><a href=tvshow.php?pid=$id>" . $row["vid"] . "</a></td>";
      if ($user_type != NULL) {
        echo "<td>";
