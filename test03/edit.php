<?php
  ...
  //要使用SQL的SELECT指令找出要被編輯的對象
  $sql = "SELECT * from playlist WHERE id ='$id' LIMIT 1";//LIMIT用來設限不要刪太多
  //以下執行SQL查詢指令，並把結果傳回給$result變數
  $result = $conn->query($sql);
  if ($result->num_rows > 0) { //檢查記錄的數量，看看是否有資料
    $row = $result->fetch_assoc();//從資料庫中取出一筆記錄
    $id=$row["id"];//指定各訊息的id
    $message=$row["name"];//取出message欄位，放到$message中
    //以下建立一個用來編輯內容的表單，按下修改後會前往update.php
    echo "以下是找到的清單，請修改<br>";
    echo "<form method=POST action=update.php>";
    echo "<input type=hidden value='$id' name=id>";//隱藏的欄位拿來放id
    echo "訊息<input type=text value='$message' name=name size=30>";
